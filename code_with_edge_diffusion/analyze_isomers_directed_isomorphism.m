...
% counts the number of isomers of each type generated (for the case with
...
for j=pore_size_list
    ...
    dirname = ['../catalog/with_edge_diffusion/pore',num2str(j)];
    adjmatListing = dir([dirname,'/adjmat_directed_antimol_*.txt']);
    ...
    for i=1:size(adjmatListing,1)
        ...
        xyzFilename =  strrep(antimolAdjmatFilename,'adjmat_directed_antimol_','pore');
        xyzFilename =  strrep(xyzFilename,'.txt','.xyz');
        txtFilename =  strrep(xyzFilename,'.xyz','.txt');

        % Get the index of the pore        
        indexCurrentPore = strrep(adjmatListing(i).name,'adjmat_directed_antimol_','');
        ...
        copyfile(antimolAdjmatFilename,[dirname,'/isomers/adjmat_directed_antimol_',sprintf('%.3d',j),'_',sprintf('%.3d',isomerNumber),'_',sprintf('%.3d',copyNumber),'.txt']);
